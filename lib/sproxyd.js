...
class SproxydClient {
    ...
    _createRequestHeader(method, key) {
        if (!key) {
            key = crypto.randomBytes(19).toString('hex').toUpperCase();
        }
    ...
    _failover(method, keysOrValue, tries, callback) {
        let counter = tries;
        this._handleRequest(method, keysOrValue, (err, ret) => {
            if (err && !err.isExpected) {
                ...
                return this._shiftCurrentBootstrapToEnd()
                    ._failover(method, keysOrValue, counter, callback);
            }
    ...
    put(value, callback) {
        ...
        this._failover('PUT', value, 0, callback);
    }
