...
class SproxydClient {
    ...
    _handleRequest(method, keysOrValue, log, callback, params) {
        if (method === 'PUT') {
            ...
            async.map(indexes, function putChunks(index, cb) {
                ...
                log.debug(`starting PUT chunk to sproxyd chunkStart=${index}`);
                ...
                const request = _createRequest(req, log, (err) => {
                    ...
                    log.debug(`stored chunk to sproxyd chunkStart=${index} ` +
                              `key=${key}`);
                    return cb(null, key);
                });
                request.write(chunk);
     ...
     */
    put(value, params, reqUids, callback) {
        ...
        this._failover('PUT', value, 0, log, callback, params);
    }
