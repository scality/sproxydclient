...
class SproxydClient {
    ...
    constructor(opts) {
        ...
        const freeSocketTimeoutConfiguration = Number(process.env.SPROXYD_CLIENT_FREE_SOCKET_TIMEOUT_MS) || 60 * 1000;
        const keepAliveTimeoutConfiguration = Number(process.env.SPROXYD_CLIENT_KEEPALIVE_TIMEOUT_MS) || 2 * 60 * 1000;
        const useNativeAgent = process.env.SPROXYD_CLIENT_USE_NATIVE_AGENT === 'true';
        if (useNativeAgent) {
            this.httpAgent = new http.Agent({ keepAlive: true });
        } else {
            this.httpAgent = new httpAgent.Agent({
                freeSocketTimeout: freeSocketTimeoutConfiguration,
                timeout: keepAliveTimeoutConfiguration,
            });
        }
