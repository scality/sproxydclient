...
function _createRequest(req, log, callback) {
    return http.request(req, response => {
...
class SproxydClient {
    ...
    _createRequestHeader(method, key, params) {
        const currentKey = key || keygen(params);
    ...
    _handleRequest(method, stream, key, log, callback, params) {
        if (stream) {
            ...
            pass.on('end', () => {
                hash.end();
                reqStream.calculatedHash = hash.read();
            });
            reqStream.pipe(pass).pipe(hash);
            reqStream.pipe(request);
