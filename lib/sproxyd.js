...
class SproxydClient {
    ...
    constructor(opts) {
        ...
        this.logging.info('bootstrap list', { bootstrap: this.bootstrap });
        if (options.chordCos) {
        ...
        this.logging.info('current bootstrap',{
            currentBootstrap: this.current,
        });
    ...
    _failover(method, stream, key, tries, log, callback, params) {
        ...
        let counter = tries;
        log.info('request', { httpMethod: method, value, args, counter });
    ...
    _handleRequest(method, stream, key, log, callback, params) {
        ...
        if (stream) {
            ...
            const request = _createRequest(req, log, (err) => {
                ...
                // We return the key
                log.debug('stored to sproxyd', { sproxydKey: newKey });
     ...
     * @returns {undefined}
     */
    put(stream, params, reqUids, callback, keyScheme) {
        assert(stream.readable, 'stream should be readable');
        const log = this.createLogger(reqUids);
        this._failover('PUT', stream, keyScheme, 0, log, (err, key) => {
            if (err)
                return callback(err);
    ...
    getHEAD(key, reqUids, callback) {
        ...
        assert.strictEqual(key.length, 40);
        const log = this.createLogger(reqUids);
        this._failover('HEAD', null, key, 0, log, (err, res) => {
            if (err) {
                return callback(err);
            }
            else {
