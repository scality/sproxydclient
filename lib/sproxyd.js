...
function _createRequest(req, log, callback) {
    ...
    const request = http.request(req, response => {
        req_socket.removeAllListeners('timeout');
    ...
    .on('socket', socket => {
        // This timeout, instead of the request one, is used to
        // timeout the socket in case of inactivity, like if the
        // client upload less than content-length, if the stream
        // is destroyed, etc, this will prevent any socket remaining
        // too long
        req_socket = socket;
        socket.setTimeout(2000);
        socket.on('timeout', () => {
            socket.removeAllListeners('timeout');
            // Using destroy() is the standard way to forward error in streams
            request.destroy(new Error('Timeout on the socket'));
        });
    })
    .on('error', err => {
        if (req_socket) {
...
class SproxydClient {
    ...
    _failover(method, stream, size, key, tries, log, callback, params,
        ...
        this._handleRequest(method, stream, size, key, log, (err, ret) => {
            if (err && !err.isExpected) {
                ...
                if (stream) {
                    // prevent any retries on stream, since part of the stream may
                    // already been consumed, even an ECONNREFUSED will start consuming
                    // the readable stream, leading to unability to retry, still, we want
                    // to shift the current bootstrap to avoid any further issue
                    this._shiftCurrentBootstrapToEnd(log);
                    return callback(err);
                }
    ...
    _handleRequest(method, stream, size, key, log, callback, params, payload) {
        ...
        if (stream) {
            ...
            stream.pipe(request);
            stream.on('error', err => {
                // If stream on error and the request is not destroyed yet,
                // destroying with forwarding the error, this will abort the
                // request
