...
class SproxydClient {
    ...
    _failover(method, keysOrValue, tries, callback, params) {
        let counter = tries;
        this._handleRequest(method, keysOrValue, (err, ret) => {
            if (err && !err.isExpected) {
                if (++counter >= this.bootstrap.length) {
                    return callback(err);
                }
                return this._shiftCurrentBootstrapToEnd()
                    ._failover(method, keysOrValue, counter, callback, params);
            }
     ...
     */
    _handleRequest(method, keysOrValue, callback, params) {
        if (method === 'PUT') {
    ...
    get(keys, callback) {
        ...
        this._failover('GET', keys, 0, callback);
    }
    ...
    delete(keys, callback) {
        ...
        keys.forEach(key => assert(typeof key === 'string'
                     && key.length === 40, 'wrong key format'));
        this._failover('DELETE', keys, 0, callback);
    }
}
