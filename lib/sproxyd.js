...
class SproxydClient {
    ...
    _handleRequest(method, stream, size, key, log, callback, params, payload) {
        ...
        if (stream) {
            ...
            const request = _createRequest(req, log, (err, response) => {
                if (err) {
                    ...
                    if (streamingStarted || voluntaryAbort) {
                        errorProcessed.retryable = false;
                    } else {
            ...
            stream.on('error', err => {
                log.error('error from readable stream', err);
            });
        } else {
     ...
     * @returns {undefined}
     * @deprecated Use head() instead to get full headers including content-length
     */
