...
class SproxydClient {
    ...
    _failover(method, keysOrValue, tries, log, callback, params) {
        const argsStr = params === undefined ? '{}' : JSON.stringify(params);
        const valStr = method === 'PUT' ? '[data]' : keysOrValue.join(',');
        let counter = tries;

        log.debug(`failover request method=${method} ` +
                  `keysOrValue=${valStr} params=${argsStr} try=${counter}`);

        this._handleRequest(method, keysOrValue, log, (err, ret) => {
            if (err && !err.isExpected) {
                ...
                return this._shiftCurrentBootstrapToEnd(log)
                    ._failover(method, keysOrValue, counter, log, callback,
                               params);
            }
     ...
     */
    _handleRequest(method, keysOrValue, log, callback, params) {
        if (method === 'PUT') {
            const hash = crypto.createHash('md5').setEncoding('hex');
            const pass = new PassThrough;
            const req = this._createRequestHeader('PUT', null, params);
            ...
            pass.on('end', () => {
                hash.end();
                keysOrValue.calculatedMD5 = hash.read();
            });
            keysOrValue.pipe(pass).pipe(hash);
            keysOrValue.pipe(request);
    ...
    put(stream, params, reqUids, callback) {
        ...
        this._failover('PUT', stream, 0, log, (err, key) => {
            if (err)
                return callback(err);
            if (stream.calculatedMD5 !== stream.contentMD5) {
    ...
    get(keys, reqUids, callback) {
        ...
        const log = this.createLogger(reqUids);
        this._failover('GET', keys, 0, log, callback);
    }
    ...
    delete(keys, reqUids, callback) {
        ...
        keys.forEach(key => assert(typeof key === 'string'
                     && key.length === 40, 'wrong key format'));
        const log = this.createLogger(reqUids);
        this._failover('DELETE', keys, 0, log, callback);
    }
}
